...
systable_endscan(SysScanDesc sysscan)
{
	if (IsYugaByteEnabled())
	{
		if (yb_run_with_analyze_explain_dist && sysscan->ybscan && sysscan->ybscan->handle)
...
{
	if (IsYugaByteEnabled() && yb_run_with_analyze_explain_dist && sysscan->ybscan && sysscan->ybscan->handle)
		YbUpdateCatalogRpcStats(sysscan->ybscan->handle);
