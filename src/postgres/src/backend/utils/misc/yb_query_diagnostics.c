...
yb_live_explain_plans(PG_FUNCTION_ARGS)
{
	...
	while ((entry = hash_seq_search(&status)) != NULL)
	{
		...
		int			explain_plan_start_pos = 0;
		int			len = strlen(entry->explain_plan);
		while (i++ < len)
		{
			if (entry->explain_plan[i] == ' ')
			{
				if (duration_ptr_left == -1)
					duration_ptr_left = i;
				else if (duration_ptr_right == -1)
					duration_ptr_right = i;
			}

			if (entry->explain_plan[i] == '\n' && entry->explain_plan[i - 1] == ':')
				explain_plan_start_pos = i + 1;
			...
			if (entry->explain_plan[i] == '\n' && entry->explain_plan[i - 1] == '\n')
			{
				...
				char duration[duration_ptr_right - duration_ptr_left];
				memcpy(duration, entry->explain_plan + duration_ptr_left + 1, duration_ptr_right - duration_ptr_left - 1);
				duration[duration_ptr_right - duration_ptr_left - 1] = 0;

				char explain_plan[i - explain_plan_start_pos - 1];
				memcpy(explain_plan, entry->explain_plan + explain_plan_start_pos, i - explain_plan_start_pos - 2);
				explain_plan[i - explain_plan_start_pos - 2] = 0;

				values[0] = UInt64GetDatum(entry->metadata.params.query_id);
				values[1] = Float4GetDatum(atof(duration));
				values[2] = CStringGetTextDatum(explain_plan);
				tuplestore_putvalues(tupstore, tupdesc, values, nulls);
