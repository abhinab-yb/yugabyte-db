 ...
 private:
  ...
  Status EnsureCompleted(size_t count) {
    ...
    if (count > 0 && !in_flight_ops_.empty()) {
      VStartEventSpan(1, "Storage Write Request");
    ...
    for(; count && !in_flight_ops_.empty(); --count) {
      ...
      if (!status.ok() && span_open) {
        VEndEventSpan(1, "Storage Write Request");
      }
  ...
  Status SendBufferImpl(const SendInterceptor* interceptor) {
    ...
    VStartEventSpan(1, "Flushing Buffered Operations");
    ...
    VEndEventSpan(1, "Flushing Buffered Operations");
    return Status::OK();
  }
