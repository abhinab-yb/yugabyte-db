...
void AUHMetadata::set_client_host_port(const HostPort &host_port) {
  client_host_port = host_port;
}

std::string AUHMetadata::ToString() const {
  return YB_STRUCT_TO_STRING(
      top_level_node_id, top_level_request_id, query_id, current_request_id, client_host_port);
}
...
void WaitStateInfo::set_code(WaitStateCode c) {
  TRACE(auh::ToString(c));
  code_ = c;
}

WaitStateCode WaitStateInfo::get_code() const {
  return code_;
}
...
void WaitStateInfo::set_top_level_request_id(const Uuid& top_level_request_id) {
  std::lock_guard lock(mutex_);
  metadata_.top_level_request_id = top_level_request_id;
}
...
void WaitStateInfo::set_top_level_node_id(
    const Uuid &top_level_node_id) {
  std::lock_guard lock(mutex_);
...
ScopedWaitStatus::ScopedWaitStatus(WaitStateInfoPtr wait_state, WaitStateCode code)
  ...
  if (wait_state_) {
    prev_code_ = wait_state_->get_code();
...
ScopedWaitStatus::~ScopedWaitStatus() {
  if (wait_state_) {
    auto current_code = wait_state_->get_code();
    if (current_code == code_) {
      wait_state_->set_code(prev_code_);
    } else {
      VLOG(3) << __func__ << " not reverting to prev_code_: " << prev_code_ << " since "
              << " current_code: " << current_code << " is not " << code_;
    }
  }
  wait_state_ = nullptr;
